...
export default function ProgressBar({ mediaRef }: ProgressBarProps) {
  const { currentTime, duration, abLoop } = usePlayerStore();
    ...
    <div className="space-y-1">
      ...
      >
        ...
        {abLoop && safeDuration > 0 && (
          <div
            className="absolute top-0 h-full bg-primary-300/50 rounded-full"
            style={{
              left: `${(abLoop.start / safeDuration) * 100}%`,
              width: `${((abLoop.end - abLoop.start) / safeDuration) * 100}%`,
            }}
          />
        )}
        {/* Played */}
